...
\begin{quote}
\itshape « [\ldots] l'horreur de la vie, la bêtise du monde,\\
l'inclémence de la destinée [\ldots] »
\end{quote}
\begin{flushright}
...
\noindent Ce que vous allez lire n'est pas un roman. Pas non plus un roman policier ou polar, pas un essai, pas une autobiographie, pas un recueil de nouvelles, pas un pamphlet, pas un manuel de savoir-vivre, pas un album pour enfants.

\medskip
\noindent C'est un \textsc{ovni} — objet littéraire non identifié.

\medskip
\noindent Il est composé de trois ensembles distincts~: 1.~une pseudo-intrigue, effectivement dans un style polar, conduite par le narrateur-héros, Djeferson Beauvais, et agrémentée de nombreuses scènes dialoguées~; 2.~des ratiocinations sans queue ni tête du même héros-narrateur sur les sujets les plus incongrus, le plus souvent des enjeux de société (faire des enfants, les riches et les pauvres, les réseaux sociaux, etc.)~; et 3.~un appareil conséquent de mentions, références et digressions, véritable cabinet de curiosités culturelles et historiques.

\medskip
\noindent Le fil rouge de ces trois ensembles me semble être l'Horreur~— à prononcer comme Marlon Brando dans \textit{Apocalypse Now}.
...
\vspace{2\onelineskip}
\begin{flushright}
\normalfont\itshape Christophe Thiebaud
\end{flushright}
