...
\usepackage{calc}
\usepackage{wrapfig}
% Prevent wrapfig state (reduced \linewidth) from leaking across chapter boundaries
...
\ifdefined\AVECILLUSTRATIONS
  ...
  \newcommand{\iconographiewrapfig}[4]{%
    ...
    % Mesure la hauteur exacte de la figure (image + légende éventuelle) afin
    % de passer [n] à wrapfigure — wrapfig restaure alors \linewidth proprement
    % après la dernière ligne enroulée, sans laisser de fuite de \linewidth.
    \sbox\@tempboxa{\parbox{#2}{\centering#3%
      \ifx&#4&\else\\[0.2em]{\scriptsize\itshape #4}\\[0.5\baselineskip]\fi}}%
    \begin{wrapfigure}[%
      \numexpr(\number\dimexpr\ht\@tempboxa+\dp\@tempboxa\relax
               +\number\dimexpr\baselineskip\relax-1)%
  ...
  \NewDocumentCommand{\iconographiedouble}{O{b} O{5cm} O{0pt} m m O{}}{%
    ...
    \else%
      ...
      {\centering\scriptsize\itshape#6\par}%
