...
\usepackage{calc}
\usepackage{wrapfig}
% Fix: \WF@finale removes the \everypar hook but never restores \linewidth.
% Patch it to reset \linewidth=\textwidth after every wrapfig ends (naturally
% or via \WFclear), so the corruption doesn't persist beyond the wrap zone.
\makeatletter
\let\WF@@orig@finale\WF@finale
\def\WF@finale{\WF@@orig@finale\global\linewidth\textwidth}
\makeatother
