...
% Internal: open a chapter hidden from TOC, then begin chapter notes with label #1.
...
\newcommand{\startscene}{%
  \ifdefined\SANSSCENES
    \ifdefined\NOTESENFIN
      \def\chapnoteslabel{Acte~\Roman{actenum}}%
    \else
      \def\chapnoteslabel{\Roman{actenum}}%
    \fi
  \else
    \stepcounter{scenenum}%
    \ifdefined\NOTESENFIN
      \def\chapnoteslabel{Acte~\Roman{actenum}~·~Scène~\arabic{scenenum}}%
    \else
      \def\chapnoteslabel{\Roman{actenum}~·~\arabic{scenenum}}%
    \fi
  \fi
  \openchapter{\chapnoteslabel}%
  \ifdefined\SANSSCENES\else
    \addcontentsline{toc}{section}{Scène~\arabic{scenenum}}%
  \fi
}
\newcommand{\finacte}{%
  % flush notes from last scene of this act (no-op when \NOTESENFIN or \SANSSCENES)
  \ifdefined\AVECNOTES
    \ifdefined\NOTESENFIN\else
      \ifdefined\SANSSCENES\else
        \cleardoublepage
...
\newcommand{\act}[1]{\startacte#1\finacte}
