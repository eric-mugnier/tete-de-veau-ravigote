...
\newcommand{\bwimage}[2][]{%
  ...
  \begin{tikzpicture}
    ...
    %%% end{scope}
  \end{tikzpicture}%
  \ifillusbwimageaddlof\illuslofentry{}{#2}\fi%
}
...
% \illusloflabel{text}: queue a lof label for the next image entry.
%   Multiple calls before \iconographiedouble/triple are consumed in order.
%   Empty text {} falls back to caption or filename.
% \illuslofentry{caption}{filepath}: internal — pops one label from the queue.
% \@illuslofflush: discard leftover labels (used at end of double/triple).
\gdef\illuslastfile{}
\newcount\@illuslofwriteidx  \global\@illuslofwriteidx=0
\newcount\@illuslofreadidx   \global\@illuslofreadidx=0
\newif\ifillusmode           % true iff \AVECILLUSTRATIONS is defined
\newif\ifillusbwimageaddlof  % true = \bwimage auto-adds its own lof entry
\illusbwimageaddloftrue
\newcommand{\illusloflabel}[1]{%
  \global\advance\@illuslofwriteidx\@ne
  \expandafter\gdef\csname @illuslabel@\the\@illuslofwriteidx\endcsname{#1}%
}
\newcommand{\@illuslofpop}{%
  \ifnum\@illuslofreadidx<\@illuslofwriteidx
    \global\advance\@illuslofreadidx\@ne
    \expandafter\let\expandafter\@illuslofcurrent
      \csname @illuslabel@\the\@illuslofreadidx\endcsname
  \else
    \let\@illuslofcurrent\relax
  \fi
}
\newcommand{\@illuslofflush}{\global\@illuslofreadidx=\@illuslofwriteidx}
\newcommand{\@illuslofdodefault}[2]{%
  \ifx&#1&\addcontentsline{lof}{figure}{\texttt{#2}}\else\addcontentsline{lof}{figure}{#1}\fi%
}
\newcommand{\illuslofentry}[2]{%
  \ifillusmode
    \ifnum\@illuslofreadidx<\@illuslofwriteidx
      \global\advance\@illuslofreadidx\@ne
      \expandafter\let\expandafter\@illusloftmplabel
        \csname @illuslabel@\the\@illuslofreadidx\endcsname
      \ifx\@illusloftmplabel\@empty
        \@illuslofdodefault{#1}{#2}%
      \else
        \expandafter\@illuslofdodefault\expandafter{\@illusloftmplabel}{#2}%
      \fi
    \else
      \@illuslofdodefault{#1}{#2}%
    \fi
...
\ifdefined\AVECILLUSTRATIONS
  \illusmodetrue
  \setlength{\cftfigurenumwidth}{0em}
  ...
  \NewDocumentCommand{\iconographieinlineblock}{O{} O{1} m m}{%
    ...
    \ifx&#1&%
      ...
      \illusbwimageaddloffalse\centerline{#3}\illusbwimageaddloftrue%
      \illuslofentry{#4}{\illuslastfile}%
    ...
    \else
      \ifthenelse{\equal{#1}{p}}{%
        ...
        \begin{figure}[#1]
          \centering%
          \illusbwimageaddloffalse#3\illusbwimageaddloftrue%
          \illuslofentry{#4}{\illuslastfile}%
  ...
  % \iconographiedouble[height][margin]{\bwimage{img1}}{\bwimage{img2}}[caption]
  ...
  \NewDocumentCommand{\iconographiedouble}{O{5cm} O{0pt} m m O{}}{%
    \vspace{0.5\onelineskip}%
    \illusbwimageaddloffalse%
    {\setlength{\leftskip}{#2}\setlength{\rightskip}{#2}%
    \noindent\bwimage[height=#1]{#3}\hfill\bwimage[height=#1]{#4}\par}%
    \illusbwimageaddloftrue%
    \illuslofentry{#5}{#3}%
    \illuslofentry{#5}{#4}%
    \@illuslofflush%
    ...
    \else%
      ...
      {\centering\figcaptionname\itshape#5\par}%
      \vspace{0.3\onelineskip}%
    \fi%
  }
  % \iconographietriple[height][margin]{\bwimage{img1}}{\bwimage{img2}}{\bwimage{img3}}[caption]
  % Three images side by side at equal height.
  % [height] common image height (default 5cm); [margin] symmetric indent (default 0pt)
  % [caption] shared caption, omit for none.
  \NewDocumentCommand{\iconographietriple}{O{5cm} O{0pt} m m m O{}}{%
    \vspace{0.5\onelineskip}%
    \illusbwimageaddloffalse%
    {\setlength{\leftskip}{#2}\setlength{\rightskip}{#2}%
    \noindent\bwimage[height=#1]{#3}\hfill\bwimage[height=#1]{#4}\hfill\bwimage[height=#1]{#5}\par}%
    \illusbwimageaddloftrue%
    \illuslofentry{#6}{#3}%
    \illuslofentry{#6}{#4}%
    \illuslofentry{#6}{#5}%
  ...
  \newcommand{\mylistoffigures}{%
    \cleardoublepage
    \pagestyle{empty}%
    \phantomsection\addcontentsline{toc}{chapter}{Table des illustrations}%
    \vspace*{4\onelineskip}%
    {\centering\normalfont\scshape\large Table des illustrations\par}%
...
%               In ENDOFBOOKNOTES mode: also inserts a page break (skipped for first scene)
%               and updates the running header to the abbreviated scene label (e.g. "I.3").
\newif\if@firstscenenotes
\@firstscenenotestrue
\newcommand{\@notesscenebreak}{%
  \if@firstscenenotes
    \global\@firstscenenotesfalse
  \else
    \clearpage
  \fi
}
\newcommand{\scene@notes}[1]{%
  \ifdefined\AVECNOTES
    ...
    \ifdefined\ENDOFBOOKNOTES
      \edef\scene@act{\Roman{actenum}}%
      \edef\scene@sc{\arabic{scenenum}}%
      \addtoendnotes{%
        \protect\@notesscenebreak
        \protect\markboth{\scene@act}{\scene@sc}%
...
\renewcommand{\cftchapterpagefont}{\normalfont}         % page number: lining figures, not bold
...
\newcommand{\mytableofcontents}{%
  ...
  \pagestyle{empty}%
  \vspace*{4\onelineskip}%
  {\centering\normalfont\scshape\large\contentsname\par}%
  \vspace{2\onelineskip}%
  \begingroup
...
% \illuspagebreak : \newpage uniquement en présence d'illustrations.
\ifdefined\AVECILLUSTRATIONS
  \newcommand{\illuspagebreak}{\newpage}
\else
  \newcommand{\illuspagebreak}{}
\fi
